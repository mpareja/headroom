...
describe('file-api', () => {
  ...
  it('retrieves group collections', async () => {
    ...
    assert.deepStrictEqual(result, require('./data/group-collections.json'))
  })

  it('retrieves activity group', async () => {
    const result = await api.getActivityGroup(8)

    assert.deepStrictEqual(result, require('./data/activity-group-8.json')) // diff can take long on fail
  })

  it('retrieves activities in group', async () => {
    const result = await api.getActivitiesInGroup(8)

    assert.deepStrictEqual(result, require('./data/activities-in-group-8.json')) // diff can take long on fail
  })
})
